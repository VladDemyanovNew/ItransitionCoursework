    ...
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Index(string tag)
        {
            ...
            if (string.IsNullOrEmpty(tag))
                mathProblems = _mathProblemService.GetAll().Result.OrderByDescending(mp => mp.LastEditDate).ToList();
            else
                mathProblems = _mathProblemService.GetAllByTagName(tag).Result.OrderByDescending(mp => mp.LastEditDate).ToList();
